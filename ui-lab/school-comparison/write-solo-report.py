...
m=json.loads((ROOT/'luna-experiment-metrics.json').read_text(encoding='utf-8'))['solo']
u=m.get('usage') or {}
def n(v):return '未取得' if v is None else f'{v:,}'
def duration(v):
    if v is None:return '未取得'
    v=round(v);return f'{v//60}分{v%60:02d}秒'
text=f'''# Luna xhigh 単騎：条件と実行ログ
...
- 観測モデル：{m.get('observed_model')}、観測推論：{m.get('observed_effort')}
- 実配信tier：{m.get('actual_service_tier') or '未取得。priority要求だけではFast配信を確認できない。'}
- セッションID：{m.get('session_id')}
- プロンプトSHA256：{sha256((ROOT/'logs/school-luna-solo-prompt.txt').read_bytes()).hexdigest()}

## 計測

| 項目 | 値 |
| --- | ---: |
| 実行時間（自身の検証・修正を含む） | {duration(m.get('execution_seconds'))} |
| 入力トークン | {n(u.get('input_tokens'))} |
| うちキャッシュ入力 | {n(u.get('cached_input_tokens'))} |
| 出力トークン | {n(u.get('output_tokens'))} |
| うち推論出力 | {n(u.get('reasoning_output_tokens'))} |
| 入力＋出力 | {n(u.get('total_tokens'))} |
...
'''
(ROOT/'LUNA-SOLO.md').write_text(text,encoding='utf-8')
